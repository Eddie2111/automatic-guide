"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const tslib_1 = require("tslib");
const express_1 = tslib_1.__importDefault(require("express"));
const user_validation_1 = require("../../../schema/validation/user.validation");
const error_1 = require("../../../templates/error");
const login_controller_1 = tslib_1.__importDefault(require("./login.controller"));
const router = express_1.default.Router();
router
    .route("/")
    .get((req, res) => {
    res.status(400).send("Wrong Method Used");
})
    .post((req, res) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    try {
        const value = req.body;
        const filterValue = user_validation_1.userPropsLoginSchema.parse(value);
        return yield (0, login_controller_1.default)(filterValue, res);
    }
    catch (err) {
        (0, error_1.ErrorHandler)(err, res);
    }
}));
module.exports = router;
//# sourceMappingURL=route.js.map